...
/** Reference data for livestock herd: groups and animals. No accounting. */
class r12_HerdRegistryRepository(private val helper: DatabaseHelper) {
    ...
    fun getGroups(): String =
        helper.fetchAll(rdb.rawQuery("SELECT id, name, notes FROM ${DatabaseHelper.T_ANIMAL_GROUPS} WHERE isDeleted=0 ORDER BY name", null)).toString()
    ...
    fun saveGroup(json: String): String {
        return try {
            ...
            val id = if (obj.has("id") && obj.getInt("id") > 0) {
                db.update(DatabaseHelper.T_ANIMAL_GROUPS, cv, "id=?", arrayOf(obj.getInt("id").toString()))
    ...
    fun deleteGroup(id: Int): String {
        return try {
            ...
            db.update(DatabaseHelper.T_ANIMAL_GROUPS, cv, "id=?", arrayOf(id.toString()))
    ...
    fun getAnimals(groupId: Int? = null): String {
        val where = if (groupId != null) "WHERE a.isDeleted=0 AND a.groupId = $groupId" else "WHERE a.isDeleted=0"
        val sql = """
            SELECT a.id, a.tagNumber, a.name, a.breed, a.gender, a.status,
                   a.dateOfBirth, a.bookValue,
                   g.name AS groupName, dam.tagNumber AS damTag
    ...
    fun saveAnimal(json: String): String {
        return try {
            ...
            val cv = ContentValues().apply {
                ...
                if (obj.has("bookValue") && !obj.isNull("bookValue")) put("bookValue", obj.getDouble("bookValue"))
            }
            val id = if (obj.has("id") && obj.getInt("id") > 0) {
                db.update(DatabaseHelper.T_ANIMALS, cv, "id=?", arrayOf(obj.getInt("id").toString()))
                obj.getInt("id").toLong()
            } else {
    ...
    fun updateAnimalStatus(id: Int, status: String): String {
        return try {
            db.update(DatabaseHelper.T_ANIMALS, ContentValues().apply { put("status", status); put("updatedOn", System.currentTimeMillis()) }, "id=?", arrayOf(id.toString()))
    ...
    fun searchAnimals(query: String): String {
        val q = "%$query%"
        val sql = """
            SELECT a.id, a.tagNumber, a.name, a.breed, a.gender, a.status, g.name AS groupName
              FROM ${DatabaseHelper.T_ANIMALS} a
              LEFT JOIN ${DatabaseHelper.T_ANIMAL_GROUPS} g ON g.id = a.groupId
    ...
    fun getHerdSummary(): String {
        val sql = """
            SELECT COUNT(*) AS total,
             ...
             WHERE status NOT IN ('sold','dead') AND isDeleted=0
        """.trimIndent()
