...
/** Reference data for animal health events and vaccination schedules. No accounting. */
class r12_AnimalHealthRepository(private val helper: DatabaseHelper) {
    private val db  get() = helper.writableDatabase
    private val rdb get() = helper.readableDatabase

    fun getSchedules(): String =
        helper.fetchAll(rdb.rawQuery("SELECT id, name, intervalDays, notes FROM ${DatabaseHelper.T_VACCINATION_SCHEDULES} ORDER BY name", null)).toString()
    ...
    fun saveSchedule(json: String): String {
        return try {
            ...
            val cv = ContentValues().apply {
                ...
                put("intervalDays", if (obj.has("intervalDays") && !obj.isNull("intervalDays")) obj.getInt("intervalDays") else null)
                put("notes", obj.optString("notes", ""))
                put("updatedOn", System.currentTimeMillis())
            }
            val id = if (obj.has("id") && obj.getInt("id") > 0) {
                db.update(DatabaseHelper.T_VACCINATION_SCHEDULES, cv, "id=?", arrayOf(obj.getInt("id").toString()))
    ...
    fun deleteSchedule(id: Int): String {
        return try {
            db.update(DatabaseHelper.T_VACCINATION_SCHEDULES,
                ContentValues().apply { put("isDeleted", 1); put("updatedOn", System.currentTimeMillis()) },
    ...
    fun getHealthEvents(animalId: Int): String {
        val sql = """
            SELECT he.id, he.eventType, he.date, he.description, he.medication, he.dosage,
                   he.vetName, he.cost, he.nextDueDate, he.notes, vs.name AS scheduleName
    ...
    fun saveHealthEvent(json: String): String {
        return try {
            ...
            val cv = ContentValues().apply {
                ...
                put("notes",       obj.optString("notes", ""))
                put("updatedOn",   System.currentTimeMillis())
                if (obj.has("cost") && !obj.isNull("cost"))            put("cost", obj.getDouble("cost"))
                if (obj.has("scheduleId") && !obj.isNull("scheduleId")) {
                    ...
                    if (intervalDays > 0) {
                        val nextDue = LocalDate.parse(obj.getString("date"), DateTimeFormatter.ISO_LOCAL_DATE)
                            .plusDays(intervalDays.toLong()).format(DateTimeFormatter.ISO_LOCAL_DATE)
            ...
            Log.e("r12AnimalHealth", "saveHealthEvent failed", ex)
            JSONObject().put("error", ex.message).toString()
        }
    }
    ...
    fun deleteHealthEvent(id: Int): String {
        return try {
            db.update(DatabaseHelper.T_ANIMAL_HEALTH_EVENTS,
                ContentValues().apply { put("isDeleted", 1); put("updatedOn", System.currentTimeMillis()) },
                "id=?", arrayOf(id.toString()))
            JSONObject().put("status", "deleted").toString()
        } catch (ex: Exception) {
    ...
    fun getOverdueVaccinations(): String {
        ...
        val sql = """
            SELECT he.animalId, a.tagNumber, a.name AS animalName,
             ...
             WHERE he.nextDueDate IS NOT NULL AND he.nextDueDate <= ?
               AND a.status NOT IN ('sold','dead')
               AND NOT EXISTS (
                   SELECT 1 FROM ${DatabaseHelper.T_ANIMAL_HEALTH_EVENTS} he2
                    WHERE he2.animalId = he.animalId AND he2.scheduleId = he.scheduleId AND he2.date > he.date
               )
