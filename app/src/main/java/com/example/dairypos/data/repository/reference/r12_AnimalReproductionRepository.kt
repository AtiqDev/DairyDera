...
/** Reference data for livestock reproduction cycles. No accounting. */
class r12_AnimalReproductionRepository(private val helper: DatabaseHelper) {
    private val db  get() = helper.writableDatabase
    private val rdb get() = helper.readableDatabase

    fun getReproductionHistory(animalId: Int): String {
        val sql = """
            SELECT r.*, calf.tagNumber AS calfTag
    ...
    fun recordHeat(json: String): String {
        return try {
            ...
            Log.e("r12Repro", "recordHeat failed", ex)
    ...
    fun recordInsemination(json: String): String {
        return try {
            ...
            db.update(DatabaseHelper.T_ANIMAL_REPRODUCTION, cv, "id=?", arrayOf(obj.getInt("cycleId").toString()))
            JSONObject().put("status", "ok").toString()
        } catch (ex: Exception) {
            Log.e("r12Repro", "recordInsemination failed", ex)
            JSONObject().put("error", ex.message).toString()
        }
    }
    ...
    fun updatePregnancyCheck(json: String): String {
        return try {
            ...
            if (confirmed == 1) {
                ...
                if (animalId > 0) {
                    db.update(DatabaseHelper.T_ANIMALS, ContentValues().apply { put("status", "pregnant"); put("updatedOn", System.currentTimeMillis()) }, "id=?", arrayOf(animalId.toString()))
                }
            }
            JSONObject().put("status", "ok").toString()
        } catch (ex: Exception) {
            Log.e("r12Repro", "updatePregnancyCheck failed", ex)
            JSONObject().put("error", ex.message).toString()
        }
    }
    ...
    fun recordCalving(json: String): String {
        return try {
            ...
            db.update(DatabaseHelper.T_ANIMAL_REPRODUCTION, cycleCv, "id=?", arrayOf(cycleId.toString()))
            ...
            Log.e("r12Repro", "recordCalving failed", ex)
            JSONObject().put("error", ex.message).toString()
        }
    }
    ...
    fun getExpectedCalvings(): String {
        val sql = """
            SELECT r.id AS cycleId, r.animalId, a.tagNumber, a.name AS animalName,
             ...
             WHERE r.outcome = 'pending' AND r.pregnancyConfirmed = 1 AND r.expectedCalvingDate IS NOT NULL
    ...
    fun getActiveCycles(): String {
        val sql = """
            SELECT r.id, r.animalId, r.cycleNumber, a.tagNumber, a.name AS animalName,
                   r.heatDate, r.inseminationDate, r.inseminationType,
                   r.pregnancyCheckDate, r.pregnancyConfirmed, r.expectedCalvingDate, r.outcome
              FROM ${DatabaseHelper.T_ANIMAL_REPRODUCTION} r
              JOIN ${DatabaseHelper.T_ANIMALS} a ON a.id = r.animalId
