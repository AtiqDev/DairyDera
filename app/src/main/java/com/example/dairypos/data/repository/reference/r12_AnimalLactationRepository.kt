...
/** Reference data for livestock lactation records. No accounting. */
class r12_AnimalLactationRepository(private val helper: DatabaseHelper) {
    ...
    fun getLactationHistory(animalId: Int): String {
        val sql = """
            SELECT l.id, l.lactationNumber, l.startDate, l.dryOffDate, l.status, l.notes,
                   CASE
                     WHEN l.dryOffDate IS NOT NULL THEN CAST(julianday(l.dryOffDate) - julianday(l.startDate) AS INTEGER)
                     ELSE CAST(julianday('now') - julianday(l.startDate) AS INTEGER)
    ...
    fun getActiveLactations(): String {
        val sql = """
            SELECT l.id, l.animalId, l.lactationNumber, l.startDate,
                   ...
                   a.tagNumber, a.name AS animalName, a.breed, g.name AS groupName
              FROM ${DatabaseHelper.T_ANIMAL_LACTATION} l
              JOIN ${DatabaseHelper.T_ANIMALS} a ON a.id = l.animalId
              LEFT JOIN ${DatabaseHelper.T_ANIMAL_GROUPS} g ON g.id = a.groupId
             WHERE l.status = 'active' AND a.status NOT IN ('sold','dead')
    ...
    fun getDryCows(): String {
        val sql = """
            SELECT l.id, l.animalId, l.lactationNumber, l.startDate, l.dryOffDate,
                   ...
                   a.tagNumber, a.name AS animalName, g.name AS groupName
              FROM ${DatabaseHelper.T_ANIMAL_LACTATION} l
              JOIN ${DatabaseHelper.T_ANIMALS} a ON a.id = l.animalId
              LEFT JOIN ${DatabaseHelper.T_ANIMAL_GROUPS} g ON g.id = a.groupId
             WHERE l.status = 'dry' AND a.status NOT IN ('sold','dead')
    ...
    fun getLactationSummary(): String {
        val sql = """
            SELECT SUM(CASE WHEN l.status='active' THEN 1 ELSE 0 END) AS activeLactating,
              ...
              FROM ${DatabaseHelper.T_ANIMAL_LACTATION} l
              JOIN ${DatabaseHelper.T_ANIMALS} a ON a.id = l.animalId
             WHERE a.status NOT IN ('sold','dead') AND l.status IN ('active','dry')
        """.trimIndent()
        val arr = helper.fetchAll(rdb.rawQuery(sql, null))
        return if (arr.length() > 0) arr.getJSONObject(0).toString() else JSONObject().toString()
    }
    ...
    fun createLactation(json: String): String {
        return try {
            ...
            db.update(DatabaseHelper.T_ANIMALS, ContentValues().apply { put("status", "active"); put("updatedOn", System.currentTimeMillis()) }, "id=?", arrayOf(animalId.toString()))
            JSONObject().put("id", id).put("lactationNumber", lactationNumber).toString()
        } catch (ex: Exception) {
            Log.e("r12Lactation", "createLactation failed", ex)
    ...
    fun recordDryOff(json: String): String {
        return try {
            ...
            val cv = ContentValues().apply {
                ...
                put("status",     "dry")
                if (obj.has("notes") && !obj.isNull("notes")) put("notes", obj.getString("notes"))
                put("updatedOn",  System.currentTimeMillis())
            }
            db.update(DatabaseHelper.T_ANIMAL_LACTATION, cv, "id=?", arrayOf(lactationId.toString()))
            ...
            if (animalId > 0) {
                db.update(DatabaseHelper.T_ANIMALS, ContentValues().apply { put("status", "dry"); put("updatedOn", System.currentTimeMillis()) }, "id=?", arrayOf(animalId.toString()))
            }
            JSONObject().put("status", "ok").toString()
        } catch (ex: Exception) {
            Log.e("r12Lactation", "recordDryOff failed", ex)
            JSONObject().put("error", ex.message).toString()
        }
    }
    ...
    fun undoDryOff(lactationId: Int): String {
        return try {
            db.update(
                DatabaseHelper.T_ANIMAL_LACTATION,
                ContentValues().apply { put("status", "active"); putNull("dryOffDate"); put("updatedOn", System.currentTimeMillis()) },
                "id=?", arrayOf(lactationId.toString())
            )
            val animalId = helper.fetchScalarInt(
                "SELECT animalId FROM ${DatabaseHelper.T_ANIMAL_LACTATION} WHERE id=?",
                arrayOf(lactationId.toString())
